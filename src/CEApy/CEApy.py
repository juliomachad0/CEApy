...
class CEA:
    def __init__(self, thrust, name,path_raiz):
        # arquivo e nome do estudo
        self.caminho_raiz = path_raiz
        self.case = str(name)
    ...
    def plot_curve(self,eixo_x,eixo_y):
        os.chdir(self.caminho_raiz)
        df = pd.read_csv('cea-exec/{}.csv'.format(self.case))
        for i in df.columns:
            df.rename(columns={i: str(i).strip()}, inplace=True)
        ...
        #outputs = ['p','t','cp','mw','mach','isp','ivac','vis','cond','pran','cf','h','r_eq.ratio','phi_eq.ratio','o/f','aeat']
        plt.legend([eixo_y])
        plt.xlabel(eixo_x)
        plt.ylabel(eixo_y)
        plt.title(eixo_x+' X '+eixo_y)
        plt.show()
